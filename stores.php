 ...
 <div class='panel-group white' id='accordion' style='position:relative; width:100%;'>
   <div class='panel panel-default white' style='position:relative; width:100%;'>
	...
	while($vending = mysql_fetch_assoc($vendor_net))
	{
		...
		echo "
       <a style='font-family: Times, serif; color: #2aa591; font-size: 200%;' class='panel-title white' data-toggle='collapse' data-parent='#accordion' href='#$vendor_id'><b>$vendor_group</b></a><br>";
	}
echo"</div>
      ...
      <img src='$image' alt='' style='width:50%; height:50%'>
       <div class='panel-group white' id='accordion' style='position:relative; width:100%;'>
		...
		while ($type_set = mysql_fetch_assoc($type_twin))
		{
			...
			echo"  
		          <div id ='$point' class='collapse' style='width:100%;'><h4>$types</h4><img src='$visual' alt='' style='width:50%; height:50%'><br>";
	...
	echo "<input type='submit' class='btn btn btn-primary ladda-button' style='background-color: #2aa591; border-color: white;' data-style='zoom-in' name='Submit' value='Order Now!'></center>
           </div>     
         </form>
       </div>
     </div>";
